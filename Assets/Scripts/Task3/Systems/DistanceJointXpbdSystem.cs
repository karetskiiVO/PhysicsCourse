...
namespace Task3 {
    public class DistanceJointXpbdSystem : IEcsRunSystem {
        private EcsFilter<DistanceJoint> joints = null;
        private const int ITERATIONS = 8;

        public void Run() {
            var dt = Time.fixedDeltaTime;
            var alpha = dt <= Mathf.Epsilon ? 0f : 1f / (dt * dt);
            ...
            foreach (var idx in joints) {
                ...
                for (var iter = 0; iter < ITERATIONS; iter++) {
                    ...
                    if (invMass <= Mathf.Epsilon) {
                        break;
                    }

                    var compliance = joint.compliance;
                    var alphaTilde = alpha * compliance;
                    var deltaLambda = (-stretch - alphaTilde * joint.lambda) / (invMass + alphaTilde);
                    joint.lambda += deltaLambda;

                    var correction = deltaLambda * direction;
